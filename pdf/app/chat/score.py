...
def random_component_by_score(component_type , component_map):
    ...
    if component_type not in ["llm" , "retriever" , "memory"]:
        raise ValueError("Inavlid component_type")


    # From redis, get the hash containing the sum total scores of for the given component_type
    values = client.hgetall(f"{component_type}_score_values")

    # From redis, get the hash containing the number of times each component has been voted on
    counts = client.hgetall(f"{component_type}_score_counts")

    # Get the valid components names from the component map
...
def score_conversation(
    ...
    client.hincrby("retriever_counts", retriever , 1)

    client.hincrby("memory_score_values", memory , score)
    client.hincrby("memory_counts", memory , 1)
